...
const Content: React.FC = () => {
  ...
  const [repositoryUnit, setRepositoryUnit] = React.useState<
    'Repository' | 'Repositories'
  >('Repositories')

  const { loading, error, data } = useQuery<Query, typeQuery>(
    SEARCH_REPOSITORIES,
    { variables: { ...query } }
  )
  ...
  return (
    <div>
      ...
      <div>
        GitHub Repositories Search Result - {repositoryCount} {repositoryUnit}
      </div>
      {}
      {loading ? (
        <p>Loading...</p>
        ...
        <>
          ...
          {data.search.pageInfo.hasNextPage ? (
            <button onClick={() => goNext(data.search.pageInfo.endCursor!)}>
              Next
            </button>
          ) : null}
        </>
